...
- 本程序原理是模拟用户在Boss直聘网页上进行点击操作；Boss直聘网站每过一段时间会**发生改版**，且有可能**包含A/B实验**，这将导致本程序相关脚本失效。如果您在使用过程中遇上程序未按照预期执行的情况，请[点击这里](https://github.com/geekgeekrun/geekgeekrun/issues/new)进行反馈。
- 您所在公司可能会对您的计算机终端或网络进行**监控**，从而**审计**、**跟踪**您的行为；上级/HRBP可能会从 IT 团队处获取到监控数据，从而了解团队成员离职倾向。如果您不希望上级/HRBP了解到您正在求职，建议您**不要在您所在公司提供的计算机终端或网络上使用本程序**。
...
- 本程序**不会上报能够识别出您身份的信息**，亦**不会向您所在公司及上级/HRBP报告您的求职行为**。
- 本程序**不对您的求职过程与结果负责**，为您开聊的职位均在Boss直聘上发布，真实性是由Boss直聘负责的；请**自行甄别为您开聊的公司、认真决定是否参加面试、慎重选择Offer**。
- 请在Boss直聘上自行**屏蔽您不期望投递的公司**；如果您不希望您当前公司的其它员工看到您在Boss直聘上活跃，请**屏蔽当前公司及关联公司**。
- 由于本程序源代码开放，因此任何人都可以编译、发布本程序；别有用心人士可能会植入包括但不限于垃圾软件、下载器、间谍软件、钓鱼软件、木马程序、蠕虫程序、勒索病毒等不相关甚至有害的内容后，二次发布本程序，这将引发您的隐私泄露，亦会对本程序原版的声誉造成不利影响。因此建议从[GitHub上GeekGeekRun仓库的Release页面](https://github.com/geekgeekrun/geekgeekrun/releases)下载本程序。**使用本程序修改版导致的问题，本程序原版不负责**。
- 本程序**下载和使用**是**免费**的（不含宽带费、流量费），任何人都可以**免费**获得，**作者没有通过本程序赚到过一分钱（大写：壹分钱；阿拉伯数字：0.01 元人民币）**。如果您为了下载或使用本程序**付出了除宽带费、流量费之外的任何费用**，只能说明**您被骗了**。**本程序对此概不负责，别来找作者商讨退款、售后事宜**。
- 本程序经历过了多次测试，理论上来说大部分情况下可以正常运行，但可能也会出现测试覆盖不到位，导致程序不按预期运行的情况；如果您有顾虑，建议通过VMWare WorkStation/Fusion、VirtualBox、Hyper-V/Windows沙盒等虚拟化技术运行本程序。如果您在使用过程中遇上程序未按照预期执行的情况，请[点击这里](https://github.com/geekgeekrun/geekgeekrun/issues/new)进行反馈。
