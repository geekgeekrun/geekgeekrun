...
assert.match(traySource, /approveReply/, 'tray must approve queued HR replies through the controller')
assert.match(traySource, /rejectReply/, 'tray must reject queued HR replies through the controller')
assert.match(traySource, /待审批回复/, 'tray must expose an approval queue entry')
