...
const readNoReplySource = await read('packages/ui/src/main/flow/READ_NO_REPLY_AUTO_REMINDER_MAIN/index.ts')
...
assert.match(readNoReplySource, /readApprovalQueue/, 'read-no-reply flow must read approved reply queue')
assert.match(readNoReplySource, /markAutoReplySent/, 'read-no-reply flow must mark approved AI auto replies as sent')
assert.match(readNoReplySource, /markAutoReplyFailed/, 'read-no-reply flow must mark approved AI auto replies as failed')
assert.match(readNoReplySource, /markAutoReplyExpired/, 'read-no-reply flow must mark stale approved AI auto replies as expired')
assert.match(readNoReplySource, /consumeApprovedAutoReplyForCurrentChat/, 'read-no-reply flow must consume approved AI auto replies in worker context')
...
assert.match(readNoReplySource, /lastHeadlessLoggedChatMessageKey/, 'headless chat logs must dedupe the latest message')
