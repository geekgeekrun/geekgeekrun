...
import type { MarkAsNotSuitRecordDto, PageResult } from '@geekgeekrun/ggr-protocol'
import JobInfoSnapshot from '../../features/JobInfoSnapshot/index.vue'
import { MarkAsNotSuitReason } from '@renderer/domain/presentation-data'
import { transformUtcDateToLocalDate } from '@geekgeekrun/utils/date.mjs'
import { gtagRenderer } from '@renderer/utils/gtag'

const tableData = ref<MarkAsNotSuitRecordDto[]>([])
...
const getRowKey = (row: MarkAsNotSuitRecordDto) => {
...
async function getMarkAsNotSuitRecord() {
  try {
    ...
    const res = (await electron.ipcRenderer.invoke('get-mark-as-not-suit-record', {
      pageNo: pagination.value.pageNo,
      pageSize: pagination.value.pageSize
    })) as PageResult<MarkAsNotSuitRecordDto>
...
const selectedJobInfoForViewSnapshot = ref<MarkAsNotSuitRecordDto | null>(null)

function handleViewJobSnapshotButtonClick(record: MarkAsNotSuitRecordDto) {
...
function formatMarkReason(row: MarkAsNotSuitRecordDto) {
