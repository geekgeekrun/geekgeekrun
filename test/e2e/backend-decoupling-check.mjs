...
const api = createSupervisorApi({ versionStore: store, processManager: manager, backendClient: { request: backendRequest }, diagnostics, installer: async () => { const version = ++installAttempt === 1 ? '1.0.1' : '1.0.2'; await stage(version); return { version } } })
...
try {
  ...
  await electron.request('update.install', { deadlineMs: 5_000 })
  ...
  await assert.rejects(electron.request('update.install', { deadlineMs: 5_000 }), { code: 'INTERNAL_ERROR' }); await Promise.all([afterElectron.close(), afterMcp.close()]); await waitForBackend(); afterElectron = client(backendSocket, 'fake-electron'); afterMcp = client(backendSocket, 'fake-mcp'); await Promise.all([afterElectron.connect(), afterMcp.connect()])
